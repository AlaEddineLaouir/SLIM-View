...
def implicit_random_range_queries(domain, size, dim_size, seed=0, merge=False):
            ...
            for attrs, conditions in queries.items():
                ...
                for lb_tuple, ub_tuple in conditions:
                    ...
                    for dom in domain:
                        ...
                        # else:
                        #     query.append(hdmm.workload.Total(dom_size))
                    workload_d.append(hdmm.workload.Kronecker(query))
                workload_d_m = hdmm.workload.VStack(workload_d,attrs)
                workloads.append(workload_d_m)
            return workloads
    else:
        workloads = []
        for attrs, conditions in queries.items():
