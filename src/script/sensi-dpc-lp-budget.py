    ...
    if __name__ == '__main__':
        ...
        #         # df.to_csv("exp-PrivBayes-"+name_dataset_exp+"-"+direct_workload_name+".csv")
                ...
                for budget_rate in tqdm(np.arange(0.2,0.8,0.1),"budget-rate loop :",leave=False):
                                    rmse_g,re_g,compressions,view_times,workloads_times = SDPCube.run(data,0.05,0.9,direct_workload,0.5,epsilon,0,budget_rate,name_dataset_exp,direct_workload_name,False,80,False)
                                    data_sensi = {
                                        "NBRDIMS" : nbr_dims,
                                        "RMSE" : rmse_g,
                                        "RE" : re_g,
                ...
                result_df.to_csv("sensi-DPCEM-Score-ASSO-NB-LAP-AVG-LPB-"+args.dataset+"-dim-6.csv")
                
                result_df = None
                
                        
                # for budget_rate in tqdm(np.arange(0.2,0.8,0.1),"budget-rate loop :",leave=False):
                #                     rmse_g,re_g,compressions,view_times,workloads_times = SDPCube.run(data,0.05,0.9,direct_workload,0.5,epsilon,0,budget_rate,name_dataset_exp,direct_workload_name,False,80,True)
                #                     data_sensi = {
                #                         "NBRDIMS" : nbr_dims,
                #                         "RMSE" : rmse_g,
                #                         "RE" : re_g,
                ...
                #                     else:
                #                         result_df = pd.concat([result_df,df])
                # result_df.to_csv("sensi-DPCRS-ASSO-NB-LAP-AVG-LPB-"+args.dataset+"-dim"+str(nbr_dims)+".csv")
