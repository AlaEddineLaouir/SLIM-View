...
def run(dataset_org,lambda_,workloads, budget_sampling, epsilon,lp_budget,synth):
    ...
    for indice in tqdm(np.arange(len(workloads)), " Workload loop : "):
        ...
        for i in tqdm(np.arange(10), " Iterations loop : "):
            ...
            allo_regions = get_allocation_for_regions(qwl_df_divided,ep_lp,ep_n)
            ...
            for index,region in enumerate(qwl_regions):
                ...
                sampled_df,asso_data = create_dp_view_asso_simple(region,region_df,allocation,ep_s,ep_n,lambda_=lambda_)
...
def create_dp_view_asso_simple(region, region_df,allocation,ep_s,ep_n,lambda_):
    allocation = np.min([region_df.shape[0]-1,allocation])
    nb_cut_points = np.sum([cond.end - cond.start + 1 for cond in region.conditions])
    nb = int(np.min([nb_cut_points,math.ceil((region.size-allocation)*lambda_)]))
    if nb>0:
...
def get_allocation_for_regions(regions,ep_lp,ep_n):
            avgs = []
            sums = []
            maxs = [] # sizes
            mins = []
