...
for name_dataset_exp in tqdm(["bitcoin","traffic","nume-adult","electricity"],"Dataset loop : ",leave=True):
    ...
    if __name__ == '__main__':
        ...
        for i in tqdm(range(len(direct_workloads)),"direct-workload loop : ",leave=False):
                ...
                rmse_g,re_g,compressions,view_times,workloads_times = run_privat_tree.run(data,epsilon,direct_workload,args.dataset,direct_workload_name,True)
                data_sensi = {
                                   "RMSE" : rmse_g,
                                   "RE" : re_g,
                                   "CR" : compressions,
                ...
                df.to_csv("exp-PrivTree-"+name_dataset_exp+"-"+direct_workload_name+"-Synth.csv")

                rmse_g,re_g,compressions,view_times,workloads_times = hdp_view_run.run(data,epsilon,direct_workload,args.dataset,direct_workload_name,True)
                data_sensi = {
                                    "RMSE" : rmse_g,
                                    "RE" : re_g,
                                    "CR" : compressions,
                ...
                df = pd.DataFrame(data_sensi)
                df.to_csv("exp-HDPView-"+name_dataset_exp+"-"+direct_workload_name+"-Synth.csv")
