...
class CountTable:
    ...
    """
    def __init__(self, pd_table,mesure_table, cardinality_dict, index, domain_dict, depth=0, id=0, seed=0):
        self.id = id
        self.seed = seed
        self.prng = np.random.RandomState((id+seed) % 4294967295)
        self.pd_table = pd_table
        self.mesure_table = mesure_table
    ...
    def split_values(self, dim, ith):
        ...
        left_mesures_bool = (self.mesure_table[dim] <= self.domain_dict[dim][0] + ith)
        right_boolean = np.logical_not(left_boolean)
        right_mesures_bool = np.logical_not(left_mesures_bool)
        left_mesure = (self.mesure_table[left_mesures_bool])['Mesure']
        rigth_mesure = (self.mesure_table[right_mesures_bool])['Mesure']

        cardinality_dict[dim] = ith + 1
        # Count of zero (empty) cells
        left_zero_num = np.prod(list(cardinality_dict.values()), dtype=np.float)  - left_boolean.sum()
        left_value = implicit_ae(left_mesure, left_zero_num)
        ...
        right_value = implicit_ae(rigth_mesure, right_zero_num)
        
        ## They multiply all the cardinalities so they have the full count of the cube
        ## Since the use (0,1) cells, the need only the boolean count to compute the avg and the rest
        ## What i can do is use the same Bool index and retrive the mesure vals computes the sums and the avg
        ## based on that i can determine the Cut AE
        ## For each block i should pass his subset of mesure    
        return left_value, right_value
    
    def get_count_of_non_empty_cells(self,query):
    ...
    def get_sub_block_values(self, block_ranges):
        ...
        for dim, block_range in block_ranges:
            ...
            boolean_index = boolean_index & (self.domain_dict[dim][0] + block_range[0] <= self.mesure_table[dim]) & (self.mesure_table[dim] <= self.domain_dict[dim][0] + block_range[1])
            condition = QueryCondition(dim, block_range[0], block_range[1])
            self.update_domain(condition, domain_dict)
        return (self.mesure_table.loc[boolean_index])['Mesure'], self.count_domain_dict(domain_dict) - boolean_index.sum()
    ...
    def split(self, d, i):
        ...
        boolean_mesures_index = (self.mesure_table[d] <= self.domain_dict[d][0] + i)
        # (self.mesure_table.index.get_level_values(d) <= self.domain_dict[d][0]+i)
        
        left_table = self.pd_table.loc[boolean_index]
        left_mesure_table = self.mesure_table.loc[boolean_mesures_index]
        ...
        left_block = CountTable(left_table,left_mesure_table, left_cardinality_dict, self.index, left_domain_dict, self.depth + 1, id=2*self.id+1, seed=self.seed)
        
        right_table = self.pd_table.loc[np.logical_not(boolean_index)]
        right_mesure_table = self.mesure_table.loc[np.logical_not(boolean_mesures_index)]
        ...
        right_block = CountTable(right_table,right_mesure_table, right_cardinality_dict, self.index, right_domain_dict, self.depth + 1, id=2*self.id+2, seed=self.seed)
    ...
    def from_dataset(cls, dataset):
        ...
        return CountTable(count_table,None, cardinality_dict, dataset.domain.attrs, domain_dict)
...
def implicit_ae(data, zero_num):
    ...
    """
    sum =  np.sum(data)
    mean = sum / (len(data) + zero_num)
    sum_ab = np.sum(np.absolute(data - mean))
    return sum_ab + np.absolute(mean)*zero_num
