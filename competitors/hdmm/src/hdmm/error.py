...
def expected_error2(W, A, x,eps=np.sqrt(2), delta=0):
    """
    Given a strategy and a privacy budget, compute the expected squared error
    """
    assert delta == 0, 'delta must be 0'
    ...
    AtA1 = AtA.pinv()
    x_e = AtA * x
    x_ep = [elem + np.random.laplace(loc=0,scale=delta/eps) for elem in x_e]
...
def rootmse2(W, A, x,eps=np.sqrt(2), delta=0):
    """ compute a normalized version of expected squared error """
    return np.sqrt(expected_error2(W, A,x, eps, delta) / W.shape[0])
